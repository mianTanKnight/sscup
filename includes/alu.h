...
static inline bit one_bit_alu_(const bit input0, const bit input1, const bit cin, const ops ops_,
    ...
    const bit SUB_RET = full_adder(input0, NOT(input1), cin, &sub_cout_v);
    // NULL Is 0
    const bit NULL_RET = 0;
    const bit AND_RET = AND(input0, input1);
    const bit OR_RET = OR(input0, input1);
    const bit XOR_RET = XOR(input0, input1);
    const bit NOR_RET = NOR(input0, input1);

    // Multiplexer Of Group
    const bit group_0_sel_0 = mux2_1(XOR_RET, SUB_RET, ops_[0]);
    const bit group_0_sel_1 = mux2_1(NOR_RET, NULL_RET, ops_[0]);
    const bit group_0_sel_2 = mux2_1(AND_RET, ADD_RET, ops_[0]);
    const bit group_0_sel_3 = mux2_1(OR_RET, SUB_RET, ops_[0]);

    const bit group_1_sel_0 = mux2_1(group_0_sel_2, group_0_sel_0, ops_[1]);
    const bit group_1_sel_1 = mux2_1(group_0_sel_3, group_0_sel_1, ops_[1]);

    const bit select = mux2_1(group_1_sel_0, group_1_sel_1, ops_[2]);

    //只有 ADD和 SUB 才有进位
    const bit cout_add = AND(AND(AND(ops_[0], NOT(ops_[1])), NOT(ops_[2])), add_cout_v);
    const bit cout_sub = AND(AND(AND(ops_[0], NOT(ops_[1])), ops_[2]), sub_cout_v);
    const bit cout_slt = AND(AND(AND(ops_[0], ops_[1]), NOT(ops_[2])), sub_cout_v);
...
static inline void byte_alu_(const byte input0, const byte input1, byte ret, const ops ops_, bit *overflow) {
    const bit is_op_sub = AND(AND(ops_[0], NOT(ops_[1])), ops_[2]);
    const bit is_op_slt = AND(AND(ops_[0], ops_[1]), NOT(ops_[2]));
    ...
    const bit true_overflow = XOR(carry_into_msb, cin);
    const bit less = XOR(ret[0], true_overflow);
