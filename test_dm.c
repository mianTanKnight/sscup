...
static void model_write_u32(uint8_t model[DEFAULT_SIZE], uint32_t idx, uint32_t data, const bit be[4], bit we, bit clk) {
    if (!(we & clk)) return;
#if !DM_ALLOW_UNALIGNED
    if ((idx & 3u) != 0) return;
#endif
...
static int test_fuzz_against_golden_model(void) {
    ...
    for (int it = 0; it < DM_FUZZ_ITERS; it++) {
        ...
        uint32_t idx;
        switch (r & 7u) {
            case 0: idx = DEFAULT_SIZE + (xs32(&seed) % 32); break;               // OOB
            case 1: idx = (DEFAULT_SIZE - 1) - (xs32(&seed) % 8); break;          // near end
            default: idx = xs32(&seed) % (DEFAULT_SIZE + 16); break;              // mix
        }
        ...
        // Random mask 0..15
        bit be[4]; mask_from_u4(xs32(&seed) & 0xFu, be);
